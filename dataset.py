from torch.utils.data import Dataset #create custom dataset
import json # load json file
from PIL import Image #load image file
import logging # log runtime information, errors
import os

logger = logging.getLogger(__name__)
WORKING_PATH=r'F:\DSC_B_2024\data'

class Sacarsm(Dataset):
    def __init__(self, mode:str, text_name:str, limit=None):
    ...
    def load_data(self, mode:str, limit) -> dict:
        cnt = 0
        data_set = dict()
        if mode in["train"]:
            f1=open(os.path.join(WORKING_PATH, self.text_name, mode+r'.json'),'r', encoding='utf-8')
            dict_datas = json.load(f1)
            for data in dict_datas.values():
                ...
                if os.path.isfile(os.path.join(WORKING_PATH,"image_data",str(image)+".jpg")):
